...
public class TokenService {
    ...
    public String generateToken(String email) {
        return Jwts.builder().subject(email).issuedAt(new Date())
                .expiration(new Date(System.currentTimeMillis() + 1000L * 60 * 60 * 24 * 7)).signWith(getSigningKey())
                .compact();
    }
    ...
    public Boolean validateToken(String token, String user) {
        try {
            String extractedValue = extractEmail(token);
            if (user.equals("admin")) {
                Admin admin = adminRepository.findByUsername(extractedValue);
                if (admin != null) {
                    return true;
                }
            } else if (user.equals("patient")) {
                Patient patient = patientRepository.findByEmail(extractedValue);
                if (patient != null) {
                    return true;
                }
            } else {
                Doctor doctor = doctorRepository.findByEmail(extractedValue);
                if (doctor != null) {
                    return true;
                }
            }
